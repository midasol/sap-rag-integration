...
| 임베딩 | `gemini-embedding-2` | 3072차원 (RAG 경로에서 `task_type=RETRIEVAL_QUERY`) |
...
인제스션 경로 (Next 측)와 RAG 쿼리 경로 (ADK 측) 모두 `gemini-embedding-2`를
사용하며 `vector(3072)`을 대상으로 합니다. 모델을 변경할 경우 `GEMINI_EMBEDDING_MODEL`과
`EMBED_MODEL` 양쪽 모두 업데이트하고, `EMBED_OUTPUT_DIM`이 컬럼 타입과
일치하는지 확인하세요 — `vector(N)` 컬럼은 in-place로 ALTER할 수 없습니다.

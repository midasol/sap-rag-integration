...
| `GEMINI_EMBEDDING_MODEL` | no | `gemini-embedding-2` | インジェスション中に使用される 3072 次元埋め込みモデル |
...
| `EMBED_MODEL` | yes | `gemini-embedding-2` | RAG **クエリ**パスに使用される。`EMBED_OUTPUT_DIM` の次元のベクトルを生成する必要がある |
...
| 用途 | デフォルトモデル | 設定箇所 |
|-----|--------------|-----------|
| 埋め込み | `gemini-embedding-2` | `GEMINI_EMBEDDING_MODEL`（Next.js）/ `EMBED_MODEL`（ADK） |
| チャット / エージェント | `gemini-3.1-pro-preview` | `SAP_AGENT_MODEL`（ADK） |
| コンテンツサマリー | `gemini-3.1-pro-preview` | `GEMINI_CHAT_MODEL`（Next.js） |

埋め込みモデルは 3072 次元ベクトルを出力し、`embeddings.embedding` カラムをターゲットとします。別の次元に切り替える場合は `EMBED_OUTPUT_DIM` を更新し、新しい DB に対して `pnpm db:setup` を再実行してください（`vector(N)` カラムはインプレースで変更できません）。
